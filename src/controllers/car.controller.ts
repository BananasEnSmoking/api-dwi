...
export const deleteItemCar =async(req:Request,res:Response):Promise<Response>=>{
    if(!req.body || !req.header){
      ...
      try {
        ...
        if(!decoded) return res.status(404).json({ message:' token invalido ' })
        conn = await pool.getConnection();
        const addcar = await conn.query('delete from car where usuarios_idusuarios = ? and products_idproducts = ?',[decoded.id,req.body.idproducts]);
        await conn.commit();
        return res.status(200).json({msg:'success'})
...
export const insertPedido =async(req:Request,res:Response):Promise<Response>=>{
  if(!req.body || !req.header){
      return res.status(400).json({ msg: 'Envia toda la informacion' })
    }
    let conn:any =null;
    ...
    try {
      ...
      if(!decoded) return res.status(404).json({ message:' token invalido ' })
      conn = await pool.getConnection();
      const getCar:any = await conn.query('insert into pedido (idpedido,idpaypal,amount,date,usuarios_idusuarios)values(default,?,?,default,?)',[req.body.idpaypal,req.body.amount,decoded.id])
      await conn.commit();
      return res.status(200).json({msg:'success'})
...
export const getPedidos=async(req:Request,res:Response):Promise<Response>=>{
  if(!req.header){
    ...
    try {
      ...
      conn = await pool.getConnection();
      const result = await conn.query('SELECT * FROM pedido where usuarios_idusuarios = ?;',[decoded.id]);
      await conn.commit();
      return res.status(200).json({msg:'success',pedidos:result[0]})
